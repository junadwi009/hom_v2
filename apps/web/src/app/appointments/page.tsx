...
import { AppointmentsScheduleView } from "@/features/appointments/appointments-schedule-view";
...
export default async function AppointmentsPage() {
  ...
  const events = state.status === "ready" ? state.rows : [];

  const canManageAppointments = currentUser.permissions.includes(
    ...
    <AppointmentsScheduleView
      ...
      listSlot={
        <AppointmentsCatalogPage
          ...
          canCreateAppointment={canManageAppointments}
          canCompleteAppointment={canManageAppointments}
          canDeductSession={currentUser.permissions.includes(
            "can_manage_client_packages",
          )}
          canMarkNoShowAppointment={canManageAppointments}
