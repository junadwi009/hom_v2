...
import {
  ...
  type CreateTagFormAction,
} from "./create-tag-types";
...
export function CreateTagSheet({
  ...
  const actionWithSuccess = useCallback(
    async (previousState: CreateTagActionState, formData: FormData) => {
  ...
  return (
    <>
      <Button
        ...
        size="sm"
        title={canCreate ? undefined : "Anda tidak memiliki izin menambah tag."}
        type="button"
      >
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex justify-end">
          <button
            aria-label="Tutup formulir tambah tag"
            ...
            <form action={formAction} className="flex flex-1 flex-col">
              <div className="flex-1 space-y-5 px-5 py-5">
                ...
                <label className="block space-y-2 text-sm font-medium text-foreground">
                  <span>Warna</span>
                  <input
                    className="h-10 w-full cursor-pointer rounded-md border bg-background px-2 disabled:opacity-60"
                    defaultValue="#3f7cae"
                    name="color"
                    type="color"
                  />
                ...
                <label className="block space-y-2 text-sm font-medium text-foreground">
                  ...
                  <input
                    ...
                    maxLength={300}
                    name="description"
                    placeholder="Client pemula, baru mulai berlatih"
                    type="text"
                  />
                </label>
                <label className="block space-y-2 text-sm font-medium text-foreground">
                  <span>Tipe</span>
                  <select className={inputClass} defaultValue="custom" name="tagType">
                ...
                {state.status !== "idle" && state.status !== "success" ? (
                  <p
                    ...
                    role="alert"
                  >
                    {state.message ?? "Tag gagal ditambahkan."}
                  </p>
                ) : null}
              </div>
