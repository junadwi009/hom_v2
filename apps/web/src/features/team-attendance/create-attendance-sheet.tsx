"use client";

import { Plus, X } from "lucide-react";
import { useActionState, useCallback, useState } from "react";
import { useRouter } from "next/navigation";

import { Button } from "@/components/ui/button";

import {
  initialCreateAttendanceActionState,
  type CreateAttendanceActionState,
  type CreateAttendanceFormAction,
} from "./create-attendance-types";
import type { PractitionerOption } from "./team-attendance-loader";

const STATUS_OPTIONS = [
  { value: "present", label: "Present (Hadir)" },
  { value: "late", label: "Late (Terlambat)" },
  { value: "leave", label: "Leave (Cuti/Izin)" },
  { value: "absent", label: "Absent (Tidak Hadir)" },
];

const inputClass =
  "h-10 w-full rounded-md border bg-background px-3 text-sm text-foreground outline-none placeholder:text-foreground-muted focus:border-accent-gold focus:ring-2 focus:ring-accent-gold-muted disabled:opacity-60";

export function CreateAttendanceSheet({
  action,
  practitioners,
  canCreate = true,
}: {
  action: CreateAttendanceFormAction;
  practitioners: PractitionerOption[];
  canCreate?: boolean;
}) {
  const router = useRouter();
  const [open, setOpen] = useState(false);

  const actionWithSuccess = useCallback(
    async (previousState: CreateAttendanceActionState, formData: FormData) => {
      const result = await action(previousState, formData);
      if (result.status === "success") {
        setOpen(false);
        router.refresh();
      }
      return result;
    },
    [action, router],
  );

  const [state, formAction, pending] = useActionState(
    actionWithSuccess,
    initialCreateAttendanceActionState,
  );

  const disabled = !canCreate || practitioners.length === 0;

  return (
    <>
      <Button
        disabled={disabled}
        onClick={() => setOpen(true)}
        size="sm"
        title={
          canCreate
            ? practitioners.length === 0
              ? "Tambah practitioner aktif dulu."
              : undefined
            : "Anda tidak memiliki izin mencatat absensi."
        }
        type="button"
      >
        <Plus aria-hidden="true" className="size-4" />
        Catat Absensi
      </Button>

      {state.status === "success" ? (
        <p
          className="fixed bottom-5 right-5 z-50 rounded-md border border-green-200 bg-green-50 px-4 py-3 text-sm font-medium text-green-900 shadow-lg"
          role="status"
        >
          {state.message ?? "Absensi dicatat."}
        </p>
      ) : null}

      {open ? (
        <div className="fixed inset-0 z-50 flex justify-end">
          <button
            aria-label="Tutup formulir catat absensi"
            className="absolute inset-0 bg-stone-950/35"
            onClick={() => setOpen(false)}
            type="button"
          />
          <section
            aria-modal="true"
            className="relative z-10 flex h-full w-full max-w-md flex-col overflow-y-auto border-l bg-background-card shadow-2xl"
            role="dialog"
          >
            <header className="flex items-start justify-between gap-4 border-b px-5 py-5">
              <div>
                <p className="text-xs font-semibold uppercase tracking-normal text-amber-800">
                  Team
                </p>
                <h2 className="mt-1 text-xl font-semibold text-foreground">
                  Catat Absensi
                </h2>
                <p className="mt-2 text-sm leading-6 text-foreground-muted">
                  Tersimpan ke database studio.
                </p>
              </div>
              <Button
                aria-label="Tutup"
                onClick={() => setOpen(false)}
                size="icon"
                type="button"
                variant="ghost"
              >
                <X aria-hidden="true" className="size-4" />
              </Button>
            </header>

            <form action={formAction} className="flex flex-1 flex-col">
              <div className="flex-1 space-y-5 px-5 py-5">
                <label className="block space-y-2 text-sm font-medium text-foreground">
                  <span>Practitioner</span>
                  <select className={inputClass} name="practitionerId" required>
                    {practitioners.map((option) => (
                      <option key={option.id} value={option.id}>
                        {option.name}
                      </option>
                    ))}
                  </select>
                </label>
                <label className="block space-y-2 text-sm font-medium text-foreground">
                  <span>Tanggal</span>
                  <input className={inputClass} name="workDate" required type="date" />
                </label>
                <label className="block space-y-2 text-sm font-medium text-foreground">
                  <span>Status</span>
                  <select className={inputClass} defaultValue="present" name="status">
                    {STATUS_OPTIONS.map((option) => (
                      <option key={option.value} value={option.value}>
                        {option.label}
                      </option>
                    ))}
                  </select>
                </label>
                <div className="grid grid-cols-2 gap-3">
                  <label className="block space-y-2 text-sm font-medium text-foreground">
                    <span>Check-in</span>
                    <input className={inputClass} name="checkIn" type="time" />
                  </label>
                  <label className="block space-y-2 text-sm font-medium text-foreground">
                    <span>Check-out</span>
                    <input className={inputClass} name="checkOut" type="time" />
                  </label>
                </div>
                <label className="block space-y-2 text-sm font-medium text-foreground">
                  <span>Catatan</span>
                  <textarea
                    className={`${inputClass} h-20 resize-none py-2`}
                    maxLength={500}
                    name="note"
                    placeholder="Keterangan (opsional)."
                  />
                </label>

                {state.status !== "idle" && state.status !== "success" ? (
                  <p
                    className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm leading-5 text-red-900"
                    role="alert"
                  >
                    {state.message ?? "Absensi gagal dicatat."}
                  </p>
                ) : null}
              </div>

              <footer className="flex items-center justify-end gap-3 border-t px-5 py-4">
                <Button onClick={() => setOpen(false)} type="button" variant="secondary">
                  Batal
                </Button>
                <Button disabled={pending} type="submit">
                  {pending ? "Menyimpan..." : "Simpan Absensi"}
                </Button>
              </footer>
            </form>
          </section>
        </div>
      ) : null}
    </>
  );
}
