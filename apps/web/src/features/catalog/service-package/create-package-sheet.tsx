    ...
    <>
      ...
      {state.status === "success" ? (
        <p
          ...
          role="status"
        >
          {state.message ?? "Package ditambahkan."}
        </p>
      ) : null}
          ...
          >
            ...
            <form action={formAction} className="flex flex-1 flex-col">
              <div className="flex-1 space-y-5 px-5 py-5">
                ...
                <Field label="Tipe">
                  <select className={inputClass} defaultValue="session_pack" name="packageType">
                    <option value="intro">Intro / Trial</option>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Jumlah sesi">
                    <input className={inputClass} min={1} name="totalSessions" required type="number" />
                  </Field>
                  <Field label="Masa berlaku (hari)">
                    <input className={inputClass} min={1} name="validityDays" required type="number" />
                  </Field>
                </div>
                <Field label="Harga (Rp)">
                  <input className={inputClass} min={0} name="priceIdr" required type="number" />
                </Field>
                <Field label="Status">
              ...
              <footer className="flex items-center justify-end gap-3 border-t px-5 py-4">
                ...
                <Button disabled={pending} type="submit">
                  {pending ? "Menyimpan..." : "Simpan Package"}
                </Button>
              </footer>
            </form>
          </section>
