...
import {
  initialCreatePractitionerActionState,
  type CreatePractitionerActionState,
  type CreatePractitionerFormAction,
} from "./create-practitioner-types";
...
export function CreatePractitionerSheet({
  action,
  canCreate = true,
}: {
  action: CreatePractitionerFormAction;
  canCreate?: boolean;
}) {
  ...
  const actionWithSuccess = useCallback(
    async (previousState: CreatePractitionerActionState, formData: FormData) => {
  ...
  return (
    <>
      <Button
        ...
        size="sm"
        title={
          canCreate ? undefined : "Anda tidak memiliki izin menambah practitioner."
        }
        type="button"
      >
      ...
      {state.status === "success" ? (
        <p
          ...
          role="status"
        >
          {state.message ?? "Practitioner ditambahkan."}
        </p>
      ) : null}
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex justify-end">
          <button
            aria-label="Tutup formulir tambah practitioner"
            ...
            <form action={formAction} className="flex flex-1 flex-col">
              <div className="flex-1 space-y-5 px-5 py-5">
                ...
                {state.status !== "idle" && state.status !== "success" ? (
                  <p
                    ...
                    role="alert"
                  >
                    {state.message ?? "Practitioner gagal ditambahkan."}
                  </p>
                ) : null}
              </div>
              ...
              <footer className="flex items-center justify-end gap-3 border-t px-5 py-4">
                ...
                <Button disabled={pending} type="submit">
                  {pending ? "Menyimpan..." : "Simpan Practitioner"}
                </Button>
              </footer>
            </form>
          </section>
