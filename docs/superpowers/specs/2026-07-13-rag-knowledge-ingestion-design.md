...
- **Scanned-PDF vision fallback is deferred (not in this MVP).** §2.3 / the extractor table list a vision fallback for scanned/image-only PDFs. `pdf.ts` detects them (`needsVision`, low confidence) but does NOT yet route them to the vision model — the owner catches near-empty extractions in the review step. Wiring the OCR fallback is a documented follow-up (see `docs/PHASE_RAG_1_KNOWLEDGE_INGESTION_LOG.md` → Known follow-ups).
- **Cross-mode embedding-space caveat.** Chunks embedded in mock mode vs with a real `OPENAI_API_KEY` occupy different vector spaces; switching the key without re-publishing sources silently degrades retrieval (no error). Documented in the phase log's Known limitations; a per-source embedding-mode tag is a future improvement.
